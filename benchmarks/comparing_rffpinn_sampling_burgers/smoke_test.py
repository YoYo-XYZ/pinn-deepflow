"""Run the shared-harness smoke path for Burgers sampling variants."""
...
try:  # Direct script execution has no package context.
    from .benchmark import SCHEMES, SMOKE_CONFIG, run_suite  # noqa: E402
except ImportError:  # pragma: no cover - exercised by direct execution
    from benchmark import SCHEMES, SMOKE_CONFIG, run_suite  # type: ignore  # noqa: E402


def main() -> None:
    with tempfile.TemporaryDirectory(
        prefix="deepflow-burgers-sampling-smoke-"
    ) as directory:
        result = run_suite(SMOKE_CONFIG, Path(directory))
        assert set(result["variants"]) == set(SCHEMES)
        ...
        for scheme, values in result["variants"].items():
            ...
            print(f"{scheme}: shared-harness smoke passed")
