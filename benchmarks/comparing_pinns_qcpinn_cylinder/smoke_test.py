"""Run the shared-harness smoke path for the cylinder benchmark."""
...
try:  # Package execution.
    ...
    from benchmark import SMOKE_CONFIG, available_variants, run_suite  # type: ignore  # noqa: E402
...
def pinn_reference_smoke():
    """Exercise the canonical FEM/reference seam when the optional backend exists."""
    try:
    ...
    with tempfile.TemporaryDirectory(
        prefix="deepflow-cylinder-reference-smoke-"
    ) as directory:
        result = run_suite(
            SMOKE_CONFIG,
            Path(directory),
...
def main(argv=None):
    ...
    if args.all_setups:
        with tempfile.TemporaryDirectory(prefix="deepflow-cylinder-smoke-") as directory:
