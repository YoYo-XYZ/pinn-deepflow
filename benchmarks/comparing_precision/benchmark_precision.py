"""Shared-harness FP32/FP64 precision benchmark for Burgers flow."""
...
def build_domain(config: BenchmarkConfig = DEFAULT_CONFIG):
    """Build the Burgers domain through the shared domain builder."""
...
def build_model(config: BenchmarkConfig = DEFAULT_CONFIG):
    """Build the one PINN used by both precision variants."""
    return df.PINN(
        input_vars=["x", "y"],
        output_vars=["u"],
        width=config.width,
...
def run_suite(
    ...
    return run_precision_suite(
        ...
        build_domain,
        build_model,
        title="Burgers precision benchmark",
        prefix="burgers_precision",
