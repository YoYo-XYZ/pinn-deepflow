"""Shared training and evaluation workflow for the cavity formulations."""
...
def build_domain(
    ...
    Boundary order is left, bottom, right, top, followed by the pressure
    reference point.  For the stream-function form, ``u=psi_y`` and
    ``v=-psi_x`` turn the velocity conditions into derivative conditions.
    """
...
def resample_callback(domain):
    ...
    def resample(epoch, _model):
        if RESAMPLE_EVERY is not None and epoch > 0 and epoch % RESAMPLE_EVERY == 0:
            domain.sampling_uniform(BOUNDARY_POINTS, _area_sampling_resolutions(INTERIOR_POINTS))
...
def _evaluate_fields(model, geometry, formulation: str) -> Dict[str, np.ndarray]:
    """Evaluate model fields, deriving velocity for the stream form."""
...
def _evaluate_centerlines(model, formulation: str):
    """Evaluate derived velocity profiles on both cavity centerlines."""
...
def train_one(
    ...
    metrics = {
        "seed": seed,
        "n_params": n_params,
        "pde_residual_count": pde_residual_count,
        ...
        "pde_loss_per_equation": float(
            final_losses["pde_loss"].detach().cpu().item() / pde_residual_count
        ),
    }
...
def run_benchmark(
    ...
    """Parse options, run requested seeds, and save aggregated results."""
    ...
    print("Loss:             raw df.calc_loss_simple")
